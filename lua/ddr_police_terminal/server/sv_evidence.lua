...
net.Receive(NET.EvidenceListRequest, function(len, ply)
    ...
    if search and search ~= "" then
        if string.match(search, "^%d+$") then
            ...
            where = "description LIKE '%" .. DB:Escape(search) .. "%' OR evidence_type LIKE '%" .. DB:Escape(search) .. "%'"
        end
    end
    local countSql = "SELECT COUNT(*) as total FROM ddr_evidence WHERE " .. where
    DB:Query(countSql, function(ok, data)
        if not ok then return end
        local total = tonumber(data[1].total) or 0
        local listSql = string.format("SELECT * FROM ddr_evidence WHERE %s ORDER BY created_at DESC LIMIT %d OFFSET %d", where, limit, offset)
