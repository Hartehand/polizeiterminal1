...
net.Receive(NET.BoloListRequest, function(len, ply)
    ...
    local countSql = "SELECT COUNT(*) as total FROM ddr_bolos WHERE active=1"
    DB:Query(countSql, function(ok, data)
        if not ok then return end
        local total = tonumber(data[1].total) or 0
        local listSql = string.format("SELECT * FROM ddr_bolos WHERE active=1 ORDER BY created_at DESC LIMIT %d OFFSET %d", limit, offset)
