...
net.Receive(NET.DutyListRequest, function(len, ply)
    ...
    if search and search ~= "" then
        where = "officer LIKE '%" .. DB:Escape(search) .. "%' OR entry_type LIKE '%" .. DB:Escape(search) .. "%'"
    end
    local countSql = "SELECT COUNT(*) as total FROM ddr_duty_log WHERE " .. where
    DB:Query(countSql, function(ok, data)
        if not ok then return end
        local total = tonumber(data[1].total) or 0
        local listSql = string.format("SELECT * FROM ddr_duty_log WHERE %s ORDER BY created_at DESC LIMIT %d OFFSET %d", where, limit, offset)
